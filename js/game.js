(function () {
  ...
  function getBiomeValue(px, pz) {
    var cellX = Math.min(6, Math.floor((px / WORLD_SIZE) * 7));
    var cellZ = Math.min(1, Math.floor((pz / WORLD_SIZE) * 2));
  ...
  function getTerrainHeight(worldX, worldZ) {
    ...
    if (biome === 8) h += octaveNoise(px * 0.025, pz * 0.025, 2, 0.6) * 6;
    if (biome === 3) {
  ...
  // —— Terrain (one mesh, vertex colors for biomes) ——
  var geo = new THREE.PlaneGeometry(WORLD_SIZE, WORLD_SIZE, SEGMENTS, SEGMENTS);
  geo.rotateX(-Math.PI / 2);
  var pos = geo.attributes.position;
  var colors = [];

  for (var i = 0; i < pos.count; i++) {
    var px = pos.getX(i) + half;
    var pz = -pos.getZ(i) + half;
    var biome = getBiomeValue(px, pz);
    var h;
    if (biome === 5) {
      h = SEA_FLOOR_Y + (octaveNoise(px * 0.04, pz * 0.04, 2, 0.5) - 0.5) * 1.5;
    } else {
      h = octaveNoise(px * 0.015, pz * 0.015, 4, 0.5) * HEIGHT_SCALE;
      if (biome === 2) h += octaveNoise(px * 0.03, pz * 0.03, 2, 0.6) * 4;
      if (biome === 8) h += octaveNoise(px * 0.025, pz * 0.025, 2, 0.6) * 6;
      if (biome === 3) {
        var mn = octaveNoise(px * 0.0055, pz * 0.0055, 3, 0.6);
  ...
  // —— Trees (temperate forest only): brown trunk, green leaves; collision so you can't pass through ——
  ...
  var margin = half - 5;
  for (var tx = -margin; tx <= margin; tx += TREE_SPACING) {
    for (var tz = -margin; tz <= margin; tz += TREE_SPACING) {
      var px = tx + half;
      var pz = -tz + half;
      ...
      var wx = tx + jx;
      var wz = tz + jz;
      if (wx < -half + 2 || wx > half - 2 || wz < -half + 2 || wz > half - 2) continue;
      ...
      leaves.position.set(wx, leavesY, wz);
      leaves.castShadow = true;
      leaves.receiveShadow = true;
      treesGroup.add(leaves);
      treeColliders.push({ x: wx, y: leavesY, z: wz, r: TREE_COLLIDE_RADIUS });
      trunkColliders.push({ x: wx, z: wz, groundY: groundY, height: TRUNK_HEIGHT, r: TRUNK_RADIUS });
    }
  }
  scene.add(treesGroup);

  // —— Taiga (biome 9): coniferous trees (spruce/pine — tall trunk + dark green cone foliage) ——
  ...
  var taigaTreesGroup = new THREE.Group();
  var taigaMargin = half - 5;
  for (var ttx = -taigaMargin; ttx <= taigaMargin; ttx += TAIGA_TREE_SPACING) {
    for (var ttz = -taigaMargin; ttz <= taigaMargin; ttz += TAIGA_TREE_SPACING) {
      var tpx = ttx + half, tpz = -ttz + half;
      if (getBiomeValue(tpx, tpz) !== 9) continue;
      var tjx = (noise2D(tpx * 0.6, tpz * 0.6) - 0.5) * 2.2;
      var tjz = (noise2D(tpx * 0.6 + 77, tpz * 0.6 + 77) - 0.5) * 2.2;
      var twx = ttx + tjx, twz = ttz + tjz;
      if (twx < -half + 2 || twx > half - 2 || twz < -half + 2 || twz > half - 2) continue;
      var tGroundY = getTerrainHeight(twx, twz);
      var taigaTrunk = new THREE.Mesh(taigaTrunkGeo, taigaTrunkMat);
      taigaTrunk.position.set(twx, tGroundY + TAIGA_TRUNK_HEIGHT / 2, twz);
      taigaTrunk.castShadow = true;
      taigaTrunk.receiveShadow = true;
      taigaTreesGroup.add(taigaTrunk);
      var taigaCone = new THREE.Mesh(taigaConeGeo, taigaLeavesMat);
      taigaCone.position.set(twx, tGroundY + TAIGA_TRUNK_HEIGHT + 2.75, twz);
      taigaCone.castShadow = true;
      taigaCone.receiveShadow = true;
      taigaTreesGroup.add(taigaCone);
      treeColliders.push({ x: twx, y: tGroundY + TAIGA_TRUNK_HEIGHT + 2.75, z: twz, r: TREE_COLLIDE_RADIUS });
      trunkColliders.push({ x: twx, z: twz, groundY: tGroundY, height: TAIGA_TRUNK_HEIGHT + 5.5, r: TRUNK_RADIUS });
    }
  }
  scene.add(taigaTreesGroup);

  // —— Jungle (biome 10): tropical trees — tall trunk, broad dark green canopy ——
  ...
  var jungleTreesGroup = new THREE.Group();
  var jungleMargin = half - 5;
  for (var jtx = -jungleMargin; jtx <= jungleMargin; jtx += JUNGLE_TREE_SPACING) {
    for (var jtz = -jungleMargin; jtz <= jungleMargin; jtz += JUNGLE_TREE_SPACING) {
      var jpx = jtx + half, jpz = -jtz + half;
      if (getBiomeValue(jpx, jpz) !== 10) continue;
      var jjx = (noise2D(jpx * 0.55, jpz * 0.55) - 0.5) * 2.5;
      var jjz = (noise2D(jpx * 0.55 + 99, jpz * 0.55 + 99) - 0.5) * 2.5;
      var jwx = jtx + jjx, jwz = jtz + jjz;
      if (jwx < -half + 2 || jwx > half - 2 || jwz < -half + 2 || jwz > half - 2) continue;
      var jGroundY = getTerrainHeight(jwx, jwz);
      var jungleTrunk = new THREE.Mesh(jungleTrunkGeo, jungleTrunkMat);
      jungleTrunk.position.set(jwx, jGroundY + JUNGLE_TRUNK_HEIGHT / 2, jwz);
      jungleTrunk.castShadow = true;
      jungleTrunk.receiveShadow = true;
      jungleTreesGroup.add(jungleTrunk);
      var jungleCanopy = new THREE.Mesh(jungleCanopyGeo, jungleLeavesMat);
      jungleCanopy.position.set(jwx, jGroundY + JUNGLE_TRUNK_HEIGHT + 4.8, jwz);
      jungleCanopy.castShadow = true;
      jungleCanopy.receiveShadow = true;
      jungleTreesGroup.add(jungleCanopy);
      treeColliders.push({ x: jwx, y: jGroundY + JUNGLE_TRUNK_HEIGHT + 4.8, z: jwz, r: TREE_COLLIDE_RADIUS });
      trunkColliders.push({ x: jwx, z: jwz, groundY: jGroundY, height: JUNGLE_TRUNK_HEIGHT + 9.6, r: TRUNK_RADIUS });
    }
  }
  scene.add(jungleTreesGroup);
  ...
  for (var ax = -archMargin; ax <= archMargin; ax += REDROCK_ARCH_SPACING) {
    for (var az = -archMargin; az <= archMargin; az += REDROCK_ARCH_SPACING) {
      ...
      if (octaveNoise(apx * 0.25, apz * 0.25 + 666, 1, 0.5) > 0.58) continue;
  ...
  // —— Redrock (biome 8): layered mesas (flat-topped with visible strata) ——
  var MESA_SPACING = 32;
  var mesaLayerColors = [0x8b4520, 0xa05028, 0xb85c3a, 0xc86840, 0xd47850];
  var mesaGroup = new THREE.Group();
  var mesaMargin = half - 8;
  for (var msx = -mesaMargin; msx <= mesaMargin; msx += MESA_SPACING) {
    for (var msz = -mesaMargin; msz <= mesaMargin; msz += MESA_SPACING) {
      var mspx = msx + half, mspz = -msz + half;
      if (getBiomeValue(mspx, mspz) !== 8) continue;
      if (octaveNoise(mspx * 0.2, mspz * 0.2 + 333, 1, 0.5) > 0.55) continue;
      var msjx = (noise2D(mspx * 0.3, mspz * 0.3) - 0.5) * 10;
      var msjz = (noise2D(mspx * 0.3 + 77, mspz * 0.3) - 0.5) * 10;
      var mswx = msx + msjx, mswz = msz + msjz;
      if (mswx < -half + 6 || mswx > half - 6 || mswz < -half + 6 || mswz > half - 6) continue;
      var mesaGroundY = getTerrainHeight(mswx, mswz);
      var mesaBaseW = 14 + noise2D(mspx, mspz) * 6;
      var mesaBaseD = 12 + noise2D(mspx + 50, mspz) * 5;
      var layerH = 2.5 + (noise2D(mspx * 2, mspz * 2) - 0.5) * 0.8;
      var y = mesaGroundY;
      for (var layer = 0; layer < 5; layer++) {
        var w = mesaBaseW - layer * 2.2;
        var d = mesaBaseD - layer * 1.8;
        if (w < 4 || d < 4) break;
        var layerMat = new THREE.MeshLambertMaterial({ color: mesaLayerColors[layer] });
        var slab = new THREE.Mesh(new THREE.BoxGeometry(w, layerH, d), layerMat);
        slab.position.set(mswx, y + layerH / 2, mswz);
  ...
  var spawnMargin = half - 8;
  for (var ax = -spawnMargin; ax <= spawnMargin; ax += ANIMAL_SPACING) {
    for (var az = -spawnMargin; az <= spawnMargin; az += ANIMAL_SPACING) {
      var apx = ax + half;
      var apz = -az + half;
      ...
      var wx = ax + jx;
      var wz = az + jz;
      if (wx < -half + 4 || wx > half - 4 || wz < -half + 4 || wz > half - 4) continue;
  ...
  function animate() {
    ...
    if (pointerLocked) {
      ...
      var margin = 2;
      playerPos.x = Math.max(-half + margin, Math.min(half - margin, playerPos.x));
      playerPos.z = Math.max(-half + margin, Math.min(half - margin, playerPos.z));
